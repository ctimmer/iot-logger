...
#
# powercontrol.py
# Copyright (C) 2021 Curt Timmerman
#
# power controller is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# power controller is distributed in the hope that it will be useful,
...
POWER_CONTROL_PIN = 10       # Randomly selected TBD

#ACTIVITY_PIN = 2             # ESP8266
...
class Globals:
    ...
    def poll_wait (self) :
        ...
        if time.ticks_diff (self.poll_time_next_ms, self.current_time_ms) <= 0 :
            ...
            #print ("Poll loop OK")
            sleep_time_ms = time.ticks_diff (self.poll_time_next_ms, self.current_time_ms)
            if sleep_time_ms < 0 :
                sleep_time_ms = 0
...
class GetCommand :
    ...
    def __init__(self,
        ...
        self.s.settimeout(0)
        self.power_settings \
            = self.global_data.message_set ("powercontrol",
                                            {"power_level": initial_power_level ,
                                            "last_update_ms": global_data.get_current_time_ms ()})
        #---- Web server
...
<input type="submit" value="Update" />
    ...
    def process_request (self, request) :
        ...
        if request["method"] == "set_power_level" :
            ...
            self.global_data.message_set ("pid_settings", request["params"])
        elif request["method"] == "shutdown" :
            global_data.shutdown ()
...
class PIDControl :
    ...
    def __init__(self,
        ...
        self.global_data = global_data             # save global_data object
        self.last_update_ms = global_data.get_current_time_ms ()
        self.pid = None
#pid.SetPoint=225.0
#pid.setSampleTime(0.01)
        ...
        self.pid_settings \
            = self.global_data.message_set ("pid_control",
                                            {"power_level": 0 ,
                                             "P" : P ,
                                             "I" : I ,
                                             "D" : D ,
...
class PowerControl :
    ...
    def __init__(self ,
        ...
        self.power_level = 0.0    # Should be zero
        self.minimum_pulse_ms = MINIMUM_PULSE_WIDTH_MS
        self.standby = False
        self.standby_timeout = global_data.seconds_to_ms (standby_timeout_seconds)
    ...
    def poll_it (self) :
        ...
        current_time_ms = global_data.get_current_time_ms ()
        ...
        if global_data.active_now (self.change_ms) :
            #print ("power on/off")
            if self.power_on :
                if self.off_ms > 0 :
                    self.set_power_off ()
                    self.change_ms = self.global_data.active_next_ms (self.off_ms)
            else :
                if self.on_ms > 0 :
                    self.set_power_on ()
                    self.change_ms = self.global_data.active_next_ms (self.on_ms)
...
class PollIndicator:
    ...
    def __init__(self,
        ...
        self.active_next_ms = self.global_data.active_next_ms (0)
...
class Heartbeat:
    def __init__(self, global_data) :
        ...
        self.global_data = global_data
        self.active_interval_ms = self.global_data.minutes_to_ms (10)  # activity seconds
        self.active_next_ms = self.global_data.active_next_ms (1500)   # Send 1st message 1.5 sec
        self.client_sender_socket = socket.socket (socket.AF_INET, socket.SOCK_DGRAM)
        self.server_address_port = socket.getaddrinfo (CLIENT_CONFIG ['LOGGER_HOST'] ,
                                                        SERVER_CONFIG ['LISTENER_PORT'])[0][-1]
        #print (self.server_address_port)
    ...
    def poll_it (self) :
        ...
        #print (result_json)
        #print ("sendto:", self.server_address_port)
        bytesToSend = str.encode (result_json)      # message length
        self.client_sender_socket.sendto (bytesToSend, self.server_address_port)
        return
...
class ShutdownTimer:
    ...
    def __init__(self,
        ...
        self.run_ms = global_data.hours_to_ms (hours) \
                           + global_data.minutes_to_ms (minutes) \
                           + global_data.seconds_to_ms (seconds)
        self.start_time_ms = global_data.get_current_time_ms ()
...
class PlugInTemplate:
    def __init__(self, global_data) :
        #print ("PlugInTemplate: init")
        self.global_data = global_data             # save global_data object
        self.active_interval_ms = global_data.seconds_to_ms (5)  # every 5 seconds
        self.active_next_ms = self.global_data.active_next_ms (200) # start after 200ms

    def poll_it (self) :
        #print ("PlugInTemplate: poll_it")
        if not global_data.active_now (self.active_next_ms) :
            return
        self.active_next_ms = self.global_data.active_next_ms (self.active_interval_ms)
        #print ("PlugInTemplate: active")
        #---- Poll code goes here

    def shutdown (self) :
        print ("PlugInTemplate: shutdown")
        #---- Shutdown code goes here
...
#---- User PlugIn's
#----
get_command = GetCommand (global_data)              # Get extrnal updates
pid_control = PIDControl (global_data)              # PID controller
power_control = PowerControl (global_data)          # Sets power level
poll_indicator = PollIndicator (global_data ,       # Indicates running status
                                xpos=0 ,
                                ypos=0 ,
                                color=GREEN)
shutdown_timer = ShutdownTimer (global_data,        # Shut down after time setting
                                hours = SHUTDOWN_HOURS ,
                                minutes = SHUTDOWN_MINUTES ,
                                seconds = SHUTDOWN_SECONDS)
#heartbeat = Heartbeat (global_data)
#watchdog = Watchdog (global_data)                  # Auto reset
#plugin_template = PlugInTemplate (global_data)     # Plugin template
...
poll_array = [
    ...
    power_control ,
    #heartbeat ,
    #Watchdog (global_data) ,
    PlugInTemplate (global_data)
    ]
